...
|장민석| 내용 필요 |
|조예린| 오목허브의 Front를 개발하며 생각했던 화면이 실제로 생성되고 동작하며 구축되어 가는 과정이 흥미로웠습니다. Vue.js를 통해 컴포넌트 기반 아키텍처를 적용하는 과정에서 코드의 재사용성과 유지보수성에 대한 이해도가 크게 높아졌습니다. 각각의 컴포넌트를 독립적으로 개발하고 조합함으로써 코드의 가독성을 높일 뿐만 아니라, 수정이 필요한 경우 해당 컴포넌트만 수정함으로써 전체적인 시스템에 미치는 영향을 최소화할 수 있었습니다. 또한, 컴포넌트 간의 통신 방법을 익히면서 데이터 흐름을 더욱 명확하게 이해할 수 있었습니다. 뿐만 아니라 Vue Router를 활용하여 페이지 간 이동을 관리하는 방법을 익히는 것 또한 매우 유익했습니다. 싱글 페이지 애플리케이션의 구축을 위해 필요한 라우팅 기능을 Vue Router가 제공함으로써, 사용자 경험을 향상시키는 데 중요한 역할을 수행할 수 있었습니다. 새로운 페이지를 재구축하는 것이 아닌 현재의 페이지를 동적으로 렌더링되게 하여 전체적인 트래픽을 감소시키고 반응성 향상을 경험하는 좋은 기회였습니다. |
|지현근| 내용 필요 |
