...
<img src="https://github.com/beyond-sw-camp/be04-3rd-OmokNoonE-OmokFront/blob/main/img/%E1%84%90%E1%85%A6%E1%84%89%E1%85%B3%E1%84%90%E1%85%B3%E1%84%80%E1%85%A8%E1%84%92%E1%85%AC%E1%86%A8%E1%84%89%E1%85%A5.png"/>
...
|&nbsp;&nbsp;팀&nbsp;원&nbsp;&nbsp;&nbsp;|회고록|
|:---:|---|
|최종찬| OmokHub의 Front를 구현하는 이번 프로젝트는 팀원들과 재미있고 창의로운 아이디어를 공유하는 기회이자, HTML, CSS, JavaScript, Vue를 활용할 수 있는 좋은 기회였습니다.<br>팀원들과 소통하며, 상상하던 웹 페이지를 직접 구현해 나가는 과정이 즐거웠으며, 데이터가 실제 웹 화면으로 이동하는 과정을 직접 구현하는 새로운 경험을 쌓을 수 있었습니다.<br>이번 단계에서는 Json 파일을 통한 직접 통신 환경으로 구축했지만, 향후에는 이전에 구현한 OmokHub의 DB 및 Backend와 통합하여, 실질적인 서비스를 제공하는 통합 OmokHub를 구현할 것입니다.<br>  |
|이재원| 처음 진행해 본 프론트엔드 프로젝트는 팀원들이 상상했던 화면을 실제로 만들어보는 좋은 기회를 제공했습니다. 하지만, 예상치 못한 여러 문제에 직면하게 되었고, 이 문제들을 해결하는 과정에서 프론트엔드의 매력을 느낄 수 있었습니다. 많은 문제들이 발생하고 해결되었지만, 결국 프론트엔드에서 우리 팀원들이 가장 주의 깊게 다룬 것은 사용자 경험과 확장성을 고려한 개발이었습니다. <br><br> 첫째로, 화면 크기에 따라 현재 구현 중인 화면이 어떻게 변화할지에 대한 문제는 기능적으로는 비교적 쉽게 처리할 수 있었지만, 사용자 입장에서 어떻게 화면이 구성됐을 때 편안함을 느끼는지를 고려해야 했습니다. 둘째로, 반응형 웹을 구현할 때, 버튼 클릭 시 나타나야 할 효과, 버튼의 위치, 한 번에 보여줘야 하는 데이터의 양 등을 처리하는 데 상당한 시간이 소요되었습니다. 개발자로서 프론트엔드 프로젝트에 참여함으로써, Vue 3를 활용한 비동기 통신이나 반응형 버튼 기능 구현 등이 많은 학습 기회가 되었지만, 벡엔드와 달리 시각적으로 직접 보여지는 화면에 기능 이외의 많은 고민이 필요하다는 것을 깨달았습니다. <br><br> 또한, 팀원들과 화면을 구성하며 각자의 생각을 함께 고민하고 해결해 나가면서, 다른 팀원이 어떤 방식으로 기능을 구현하고 싶은지, 코드를 어떻게 통일화하고 싶은지에 대한 토론을 통해 이전 벡엔드 프로젝트와 비교했을 때 더 성숙해졌다고 느꼈습니다. 이를 통해 성장하고 있다는 긍정적인 인식을 얻을 수 있었습니다. 프로젝트를 함께 진행한 팀원들에게 감사의 마음을 전하며, 앞으로도 계속해서 발전해 나갈 수 있기를 바랍니다.|
|장민석| 프론트 프로젝트를 시작하면서 많은 것을 배웠습니다. HTML5를 배우면서 처음으로 시맨틱 태그를 알게 되었고, 화면 구성을 위해 div 태그를 활용하는 방법도 익혔습니다. 이러한 지식을 적극적으로 활용하여 화면을 구성했습니다. 기본적인 JavaScript 문법을 다뤄보았지만, ES6 문법이나 비동기 처리에 대한 이해는 처음이었습니다.<br><br>비동기 처리는 요청과 응답이 동시에 이루어지지 않고, 요청에 대한 응답이 도착할 때까지 기다리지 않고 다른 작업을 수행하는 것을 의미합니다. CSS와 Vue의 기본 사용법을 배운 후에는 빠르게 Vue 프로젝트를 진행했습니다. 프로젝트 마감일자가 얼마남지 않아 프론트 프로젝트라고 생각되어 백엔드와 연결을 고려하지않고 HTML, CSS, JavaScript, Vue에 집중하기로 했습니다. 그 결과로 디자인과 시맨틱 구조를 중점적으로 활용하여 Vue 페이지를 만들었습니다.<br><br>JavaScript나 Vue에서는 빠르게 진행됐지만, CSS는 처음 접하는 부분이라 진행 속도가 느렸습니다. 그러나 팀원들과 함께 노력하여 프로젝트를 완성했습니다. 이 프로젝트는 아쉬운 점이 있었습니다. 디자인과 HTML 구조에 덜 신경쓰고 기능에 초점을 맞출 수밖에 없었던 것이 그랬습니다. 또한, 원래 계획보다 더 많은 페이지를 만들지 못한 점이 아쉬웠습니다.<br><br>하지만 이 경험을 통해 다음에는 기능 구현에 더 집중해보고 싶습니다. 처음 접해보는 부분이라 어느 정도 시간이 걸렸지만, 이제는 JavaScript 뿐 아니라 HTML과 CSS도 자신 있게 구현할 수 있게 되었습니다. 다음 비슷한 프로젝트에는 기능 구현에 집중하여 더 나은 결과물을 만들어보고 싶습니다. |
|조예린| 오목허브의 Front를 개발하며 생각했던 화면이 실제로 생성되고 동작하며 구축되어 가는 과정이 흥미로웠습니다. Vue.js를 통해 컴포넌트 기반 아키텍처를 적용하는 과정에서 코드의 재사용성과 유지보수성에 대한 이해도가 크게 높아졌습니다. 각각의 컴포넌트를 독립적으로 개발하고 조합함으로써 코드의 가독성을 높일 뿐만 아니라, 수정이 필요한 경우 해당 컴포넌트만 수정함으로써 전체적인 시스템에 미치는 영향을 최소화할 수 있었습니다.<br> 또한, 컴포넌트 간의 통신 방법을 익히면서 데이터 흐름을 더욱 명확하게 이해할 수 있었으며 Vue Router를 활용하여 페이지 간 이동을 관리하는 방법을 익히는 것 또한 매우 유익했습니다. 싱글 페이지 애플리케이션의 구축을 위해 필요한 라우팅 기능을 Vue Router가 제공함으로써, 사용자 경험을 향상시키는 데 중요한 역할을 수행할 수 있었습니다. 새로운 페이지를 재구축하는 것이 아닌 현재의 페이지를 동적으로 렌더링되게 하여 전체적인 트래픽을 감소시키고 반응성 향상을 경험하는 좋은 기회였습니다. |
|지현근| Front 프로젝트를 진행하면서 다양한 Frontend 기술들이 존재한다는 것을 깨달았으며 특히 Vue3를 활용하여 프로젝트를 진행하면서 새로운 기술들을 배우고 적용하는 과정에서 흥미를 느꼈으며 뿌듯함을 느꼈습니다. 또한 Backend와 마찬가지로 팀원들간의 Ground Rule을 토대로한 협업이 프로젝트 진행에 있어서 굉장히 중요하다고 느꼈습니다. 각자의 역할을 나누고 프로젝트를 진행하면서 많은 소통을 통해 문제를 해결해나가는 과정에서 효율성과 결과의 완성도가 향상되는 것을 느꼈습니다. 이번 프로젝트에서는 Backend와의 연동을 경험해 보지 못했기 때문에 다음프로젝트에서는 기술적, 내용적 고도화를 통해 Backend와 연동하는 과정을 경험하고 싶다는 생각이 들었습니다. 이번 프로젝트를 통해 얻은 경험을 토대로 더 많은 학습을 하여 다음 프로젝트에는 팀에 더 기여하는 팀원이 되어 완성도 높은 프로젝트를 진행해보고 싶습니다.
